...
public class FieldErrorAttributeProcessor extends AbstractAttributeTagProcessor {
	...
	protected String fieldErrorClass(IProcessableElementTag tag) {
		...
		return tag.getAttribute("error-css").getValue();
	}
	/**
	 * If Type-Conversion Error found at Struts2, overwrite request-parameter same name.
	 *
	 * @param fieldname parameter-name
	 * @return request-parameter-value(if conversion error occurs,return from struts2 , not else thymeleaf.)
	 */
	protected String getOverwriteValue(String fieldname) {
		...
		String conversionValue = (String)overrideMap.get(fieldname);

		// Struts2-Component is wrapped String quote, which erase for output value.
		String altString =  StringEscapeUtils.unescapeJava(conversionValue);
